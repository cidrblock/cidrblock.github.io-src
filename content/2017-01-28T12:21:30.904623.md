...
That was basically how the conversation played out.  The request was simple, add a DHCP where DHCP helpers exist, on every device.

This is part of the brownfield conversion process.  The devices are still their own source of truth and are not fully managed by Ansible. There just isn't time every time to do it 100% right, but that doesn't mean we won't do it Ansible. The steps down the road can be small as long as they are in the right direction.
...
- A technique that keeps a running log of actual or proposed changed throughout the entire playbook, and produces a change long at the end.  This helps in the ITSM change process.

- A notify/handler that only saves the configuration once at the end of the playbook if a device was changes.
- OS specific tasks and files, "do this is it is nxos, do this if it is ios"
...
and get flagged when there is a change:
[roles/add_helpers/tasks/ios_interface.yml](https://github.com/cidrblock/add_dhcp_helper_address/blob/master/roles/add_helpers/tasks/ios_interface.yml#L9)
