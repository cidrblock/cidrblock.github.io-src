Title: Section snipping module for Ansible
Date: 2017-01-23T15:21:03.859019
Modified: 2017-01-23T15:21:03.859019
Category: ansible
Tags: ansible, netdevops
Slug:
Authors: Bradley A. Thornton
Summary: Need a section snipped from a config?  Place this in your library and:

```yaml
- name: Show matching sections
  get_section:
    lines: "{{ running_config }}"
    match: "^ip access-list standard"
    child_match: any
    children_that_match: False
    min_child_lines: 0
  register: matches
...
def find_sections(**kwargs):
    ...
    Returns:
        list: A list of the sections, children
    """
    ...
    for section in found_objs:
        ...
        for child in section.children:
            entry['section_content'].append(child.text)
...
def main():
    ...
    try:
        sections = find_sections(lines=module.params['lines'],
        ...
        module.exit_json(changed=False, section=sections)
